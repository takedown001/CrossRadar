...
public class FloatLogo extends Service implements View.OnClickListener {
    ...
    public void onCreate() {
        ...
        createOver();
        Auth();
        myDaemon = "./"+ Instance.getFilesDir().toString()+urlref.nameoflib;
    ...
    public  void Start(final Context context,final int game,final int bit) {
...
//Toast.makeText(context,"Already Started !!",Toast.LENGTH_LONG).show();
    ...
    public void StartDaemon(final Context context,int gameType){
        String pathf = getFilesDir().toString()+"/lib-native.so";

        ShellUtils.SU(pathf + " "+gameType);
        ShellUtils.SU(myDaemon + " 13122002");
	   //     Log.d("test",pathf+" "+gameType);
        //Toast.makeText(context,"Already Started !!",Toast.LENGTH_LONG).show();
    }
                        ...
                        new Thread(new Runnable() {
                            ...
                            public void run() {
                                try {
                                    ...
                                    params.put(TAG_KEY, SplashScreenActivity.key);
                                    params.put(TAG_DEVICEID, Helper.getUniqueId(Instance));
                                    String s = jsonParserString.makeHttpRequest(urlref.login, params);

                                    if (s == null || s.isEmpty()) {
                                        Toast.makeText(Instance, "Server Error", Toast.LENGTH_LONG).show();
                                        return;
                                    } else {
                                        ...
                                        if (!Helper.verify(decData, ack.get("Sign").toString(), JSONParserString.publickey)) {
                                            Toast.makeText(Instance, "Something Went Wrong", Toast.LENGTH_LONG).show();
                                            return;
                                        } else {
                                            JSONObject obj = new JSONObject(decData);
                                            time = obj.getInt("duration");
    ...
    public void gamerun(){
                ...
                if (gameName == 1 && gameType== 1) {
                    Start(Instance,1,1);
                }
                else if (gameName == 1 && gameType== 2) {
                    Start(Instance,1,2);
                }
                else if (gameName == 2 && gameType== 1) {
                    Start(Instance,2,1);
                }
                else if (gameName == 2 && gameType== 2) {
                    Start(Instance,2,2);
                }
                else if (gameName == 3 && gameType== 1) {
                    Start(Instance,3,1);
                }
                else if (gameName == 3 && gameType== 2) {
                    Start(Instance,3,2);
                }
                else if (gameName == 4 && gameType== 1) {
                    Start(Instance,4,1);
                }
                else if (gameName == 4 && gameType== 2) {
                    Start(Instance,4,2);
                }
    }
    ...
    public int onStartCommand (Intent intent, int flags, int startId) {
        gameName = intent.getIntExtra("gamename",1);
        version = intent.getStringExtra("gametype");
    ...
    private String getType(){
        SharedPreferences sp=Instance.getSharedPreferences("espValue",Context.MODE_PRIVATE);
